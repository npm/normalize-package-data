    ...
    ;['dependencies','devDependencies'].forEach(function(deps) {
      ...
      Object.keys(data[deps]).forEach(function (d) {
        ...
        if (typeof r !== 'string') {
          ...
          var v = semver.validRange(data[deps][d], loose)
          if (v === null) {
            // if it's null, then it MUST be either a name/project or a url
            v = url.parse(data[deps][d])
            if (v.protocol) return
            else if (data[deps][d].match(/^[^\/]+\/[^\/]+$/)) return
            var m = 'Invalid dependency: ' + d + '@' + JSON.stringify(r)
